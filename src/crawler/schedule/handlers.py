...
from src.crawler.schedule.crawler import crawl_schedules
            ...
            async with AsyncSessionLocal() as db:
                if student:
                    ...
                    try:
                        ...
                        if schedules:
                            logger.info(f"Successfully crawled {len(schedules)} schedules for {student['nickname']}")
                            ...
                            error_msg = f"No schedules retrieved for {student['nickname']}"
                            logger.error(error_msg)
                            await bot.send_message(settings.admin_chat_id, f"Error: {error_msg}")
                    ...
                    logger.warning(f"Received crawl event at {timestamp} without student information")
