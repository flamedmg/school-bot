...
class Settings(BaseSettings):
    ...
    # Field validators
    @field_validator('telegram_api_id', 'telegram_chat_id', 'api_port', 'api_workers', mode='before')
    ...
    @field_validator('telegram_api_hash', 'telegram_bot_token', 
                    'school_website_url', 'school_email_server', mode='before')
    ...
    def students(self) -> List[StudentConfig]:
        ...
        if not students:
            raise ValueError("At least one student must be configured using STUDENT_EMAIL_<nickname> and STUDENT_PASSWORD_<nickname>")
