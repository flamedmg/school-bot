...
from . import models
from ..crawler.schedule.schema import (
...
class ScheduleRepository:
    ...
    def get_schedule_by_unique_id(self, unique_id: str, nickname: str) -> Optional[models.Schedule]:
    ...
    def _check_lesson_order(self, new_lessons: List[Lesson], db_lessons: List[models.Lesson]) -> bool:
    ...
    def _check_lesson_marks(self, new_lessons: List[Lesson], db_lessons: List[models.Lesson]) -> List[Dict[str, Any]]:
    ...
    def _check_lesson_subjects(self, new_lessons: List[Lesson], db_lessons: List[models.Lesson]) -> List[Dict[str, Any]]:
    ...
    def _check_announcements(self, new_announcements: List[Announcement], db_announcements: List[models.Announcement]) -> Dict[str, List[str]]:
        ...
        return {
            'added': list(new_ids - db_ids),
            'removed': list(db_ids - new_ids)
        }
    ...
    def get_changes(self, schedule: ScheduleModel) -> Dict[str, Any]:
        ...
        for day in schedule.days:
            db_day = next((d for d in db_schedule.days if d.unique_id == day.unique_id), None)
            if not db_day:
                continue
            ...
            changes['marks'].extend(self._check_lesson_marks(day.lessons, db_day.lessons))
            changes['subjects'].extend(self._check_lesson_subjects(day.lessons, db_day.lessons))

            # Check announcements
            announcement_changes = self._check_announcements(day.announcements, db_day.announcements)
