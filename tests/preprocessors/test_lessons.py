import pytest
from src.crawler.schedule.preprocessors.lessons import (
    clean_lesson_index,
    clean_topic,
    preprocess_lesson,
    preprocess_lessons,
...
def test_clean_lesson_index():
    ...
    assert clean_lesson_index("·") is None  # Special lessons like "Tautas dejas kol. 'Balaguri' (I)"
...
def test_clean_subject():
    ...
    assert clean_subject("Latviešu valoda un literatūra403") == ("Latviešu valoda un literatūra", "403")
    ...
    assert clean_subject("Tautas dejas kol. 'Balaguri' (I)") == ("Tautas dejas kol. 'Balaguri'", None)
...
def test_preprocess_lesson():
    ...
    assert processed["topic"] == "Kāda ir darbību secība izteiksmēs?SR: nosauc darbību secību pēc kārtas; atrisina izteiksmes pēc parauga."
    ...
    assert "index" in processed  # Should have an index (will be assigned by preprocess_lessons)
    assert "number" not in processed  # Old field removed
    ...
    assert "number" not in processed  # Old field removed
    assert processed["subject"] == "Sports un veselība"  # Changed: expect cleaned subject without 'sz'
...
def test_preprocess_lessons():
        ...
        "lessons": [
            {
                ...
                "homework": {
                    ...
                    "links": [{"url": "/Attachment/Get/e686f1ba-5be8-46a3-aec1-9ccfb83dba5c"}],
                    "attachments": [{
    ...
    assert lesson1["subject"] == "Latviešu valoda un literatūra"  # Updated to expect cleaned subject
...
def test_error_handling():
    ...
    invalid_lesson = {
        "number": {},  # Invalid type for number
        "subject": "Math"
    }
