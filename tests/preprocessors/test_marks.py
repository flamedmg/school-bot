import pytest
from src.crawler.schedule.preprocessors.marks import (
    convert_single_mark,
    calculate_average_mark,
    preprocess_marks
)
from src.crawler.schedule.preprocessors.exceptions import MarkPreprocessingError
...
def test_calculate_average_mark():
    ...
    # Test valid cases
    assert calculate_average_mark(["85%", "A", "P"]) == 9  # (9 + 7 + 10) / 3 = 8.67 -> 9
...
def test_preprocess_marks():
    ...
    assert processed[0]["lessons"][0]["mark"] == 9  # Now expecting integer instead of dict
