...
def test_check_announcements(
    repository, make_announcement, make_school_day, sample_date
):
    ...
    announcement2 = make_announcement(
        type=AnnouncementType.GENERAL, text="School closed tomorrow", day=day
    )
    ...
    new_day = make_school_day(date=sample_date, announcements=[announcement2])

    changes = repository._check_announcements(
...
def test_get_changes_detects_lesson_order_change(
    repository, lesson_order_day, reversed_lesson_order_day, make_schedule
):
...
def test_get_changes_detects_multiple_changes(
    repository, sample_schedule, modified_schedule
):
