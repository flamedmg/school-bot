...
from src.crawler.schedule.schedule import Schedule, SchoolDay, Lesson, Homework, Announcement, AnnouncementType, schema
...
def test_real_data_pipeline_and_changes(db_session):
    ...
    for orig_day, saved_day in zip(initial_schedule.days, saved_schedule.days):
        ...
        for orig_lesson, saved_lesson in zip(orig_day.lessons, saved_day.lessons):
            ...
            if orig_lesson.homework:
                ...
                assert len(orig_lesson.homework.attachments) == len(saved_lesson.homework.attachments)
                assert len(orig_lesson.homework.links) == len(saved_lesson.homework.links)
        ...
        for orig_ann, saved_ann in zip(orig_day.announcements, saved_day.announcements):
            assert orig_ann.type.value.upper() == saved_ann.type  # Compare uppercase values
    ...
    modified_day = next(d for d in loaded_modified.days if d.date == modified_schedule.days[0].date)
    assert modified_day.lessons[0].mark == 9
    assert modified_day.lessons[1].subject == "Modified Subject"
    assert len(modified_day.announcements) == len(modified_schedule.days[0].announcements)
