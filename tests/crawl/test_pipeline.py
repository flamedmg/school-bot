...
from src.crawler.schedule.schedule import (
    Schedule, SchoolDay, Lesson, Homework, 
    Attachment, Link, Announcement, AnnouncementType,
    schema
)
...
def test_schedule_pipeline_output(capsys):
    ...
    try:
        ...
        pytest.fail(f"Failed to validate pipeline output with Pydantic models: {str(e)}")
